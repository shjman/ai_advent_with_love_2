...
class SendMessageUseCase(private val repository: ClaudeRepository) {
    suspend operator fun invoke(history: List<ChatMessage>, maxTokens: Int, stopSequence: String?, systemPrompt: String?): Result<ChatMessage> =
        repository.sendMessage(history, maxTokens, stopSequence, systemPrompt)
}
